...
pub fn add_user(newuser: database::NewUser) -> Result<NewUserResponse>{//return userid
    ...
    let exists:bool = crate::database::get::check_uname(newuser.username.clone()).unwrap() || crate::database::get::check_email(newuser.email.clone()).unwrap();
    if exists {
    ...
    while{
        ...
        newtoken = crate::database::get::check_token(token.clone(), user.auth_id, newuser.ip.clone()).unwrap();
